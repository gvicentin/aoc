...
static struct monkey ms[] = {{{79, 98}, 0, 2, OP_MUL, true, 19, 23, 2, 3},
...
static char *testLevelOfMonkeyBusiness(void) {
    ...
    int i = 20;
    do {
        execute_round(ms, inspectedItems, 4, 3);
    } while (--i > 0);
...
static char *testLevelOfMonkeyBusiness2(void) {
    ...
    int i = 10000;
    do {
        execute_round(ms, inspectedItems, 4, 1);
    } while (--i > 0);
    ...
    qsort(inspectedItems, 4, sizeof(inspectedItems[0]), compareLevel);
    uint64_t result = inspectedItems[0] * inspectedItems[1];
    MU_ASSERT_FMT(2713310158L == result, "Expect result (%lu) but got (%lu)", 2713310158L,
                  result);
    MU_PASS;
}
